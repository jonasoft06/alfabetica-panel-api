...
/**
 * The type-dependent rules below read `type` off the validated object. Both
 * concrete DTOs declare it — optional on update, required on create — so the
 * rules are written against this minimal shape.
 */
interface PublicationTypeCarrier {
...
class RequiredForPublicationTypeConstraint implements ValidatorConstraintInterface {
  validate(value: unknown, args: ValidationArguments): boolean {
    ...
    const dto = args.object as PublicationTypeCarrier;
...
export function RequiredForPublicationType(
...
export function OptionalForPublicationType(
  scopedType: PublicationType,
  kind: ScopedFieldKind,
  validationOptions?: ValidationOptions,
) {
...
/**
 * Every writable publication field except `type`, which the create and update
 * DTOs declare with their own optionality. Keeping the fields here means both
 * endpoints validate identically and cannot drift apart.
 *
 * coverMediaId is intentionally absent: it's read-only from these endpoints,
 * same as Portfolio.coverMediaId — it's written only by the dedicated
 * cover flow.
 */
export abstract class PublicationFieldsDto {
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
  authors?: string[];
